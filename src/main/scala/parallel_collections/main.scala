...
object main {
  ...
  /*def foo[A](elems: List[List[A]])(pred: A=>Boolean): Option[A]  = {
    elems.toStream.flatMap {
      alts => alts.toParArray.find(pred).headOption
    }
  }

  def expensive(a:Int): Boolean ={
    println(s"pred($a) starts")
    Thread.sleep(200)
    println(s"pred($b) ends")
    a%2==0
  }*/
